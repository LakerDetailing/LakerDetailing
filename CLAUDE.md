...
| Deo | Fajl | Šta radi |
|---|---|---|
| Sakupljanje | `assets/js/mera.js` | Učitan na SVIH 9 strana (`?v=5`). **`keepalive: true` sme SAMO na završnom paketu** — na običnom fetch-u Chrome nikad ne javi DevTools-u „završeno", pa Lighthouse/PageSpeed čekaju 45 s i prijave „strana se učitava presporo" (nađeno 2026-09-03, važilo za svih 8 strana). Šalje `pregled`, `klik`, `sekcija`, `kraj` na `/api/mera`. Od 2026-09-03 beleži i interne prelaze (`klik` naziv `ka:/cenovnik#sastavi`), izbor veličine na cenovniku (`velicina:veliki`), `paket-detalji`, a kao „sekcije" meri i `.usl-block[id]` / `.usl-aside#cena` na stranama usluga |
